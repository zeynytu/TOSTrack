...
def combine_tokens(template_tokens, search_tokens, ot_tokens = None, mode='direct', return_res=False):
    # [B, HW, C]
    len_t = template_tokens.shape[1]
    len_s = search_tokens.shape[1]

    if mode == 'direct':
        if ot_tokens is not None:
            if len(ot_tokens)==1:
                merged_feature = torch.cat((template_tokens, search_tokens, ot_tokens[0]), dim=1)
            if len(ot_tokens)==2:
                merged_feature = torch.cat((template_tokens, search_tokens, ot_tokens[0], ot_tokens[1]), dim=1)
            if len(ot_tokens)==3:
                merged_feature = torch.cat((template_tokens, search_tokens, ot_tokens[0], ot_tokens[1], ot_tokens[2]), dim=1)
            if len(ot_tokens)==4:
                merged_feature = torch.cat((template_tokens, search_tokens, ot_tokens[0], ot_tokens[1], ot_tokens[2], ot_tokens[3]), dim=1)
        else:
