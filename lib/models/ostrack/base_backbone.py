...
class BaseBackbone(nn.Module):
    ...
    def forward_features(self, z, x, ot = None):
        # z template
        ...
        if ot is not None:
            for i in range(len(ot)):
                ot[i] += self.pos_embed_z
        if self.add_sep_seg:
            x += self.search_segment_pos_embed
            z += self.template_segment_pos_embed
        if ot is not None:
            x = combine_tokens(z, x, ot, mode=self.cat_mode)
        else:
    ...
    def forward(self, z, x, ot, **kwargs):
        """
        ...
        Returns:
            ...
            attn : None
        """
        x, aux_dict = self.forward_features(z, x, ot)
